...
data_dir="${COLLECTOR_COMMODITIES_DIR}"
year=$(date +"%Y")
month=$(date +"%m")
day=$(date +"%d")
...
for attempt in $(seq 1 $max_retries); do
    echo "Downloading commodity data (attempt $attempt)..."
    curl "https://${BLIZZARD_REGION}.api.blizzard.com/data/wow/auctions/commodities?namespace=${BLIZZARD_NAMESPACE}&locale=${BLIZZARD_LOCALE}" \
...
rsync -az --remove-source-files -e "ssh -i $HOME/.ssh/id_ed25519" "$data_dir/" "$SERVER_USER@$SERVER_HOST:$SERVER_COMMODITIES_DIR/"
